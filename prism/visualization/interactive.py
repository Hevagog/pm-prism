...
class InteractiveVisualizer:
    ...
    def create_app(self) -> Dash:
        ...
        app.layout = html.Div([
            # Header
            html.H1("Process Decomposition Explorer", 
            ...
            html.Div([
                ...
                html.Div([
                    html.Label("Abstraction Level:", style={"marginRight": "15px", "whiteSpace": "nowrap"}),
                    html.Div([
                        dcc.Slider(
                            id="level-slider",
                            min=0,
                            ...
                            step=1,
                            value=0,
                            marks={i: f"L{i+1}" for i in range(max_level + 1)},
                        ),
                    ], style={"width": "400px", "minWidth": "200px"}),
                ], id="slider-container", style={"display": "flex", "alignItems": "center"}),
            ], style={"display": "flex", "alignItems": "center", "padding": "10px 40px", "justifyContent": "center"}),
            
            # Current view info
            html.Div(id="view-info", style={"textAlign": "center", "color": "#666", "marginBottom": "10px"}),
            ...
            dcc.Store(id="view-state", data={"mode": "hierarchy", "level": 0, "subprocess_id": None}),
        ])
        
        @app.callback(
            [Output("main-graph", "figure"),
             Output("main-graph", "clickData"),  # Reset clickData to allow re-clicking same node
        ...
        def update_view(click_data, back_clicks, slider_value, view_state):
            ...
            if triggered_id == "back-button":
                ...
                return fig, None, new_state, back_hidden, slider_visible, f"Showing hierarchy level {level + 1}"
            
            # Handle slider change
            if triggered_id == "level-slider":
                if view_state.get("mode") == "hierarchy":
                    fig = self._create_hierarchy_figure(slider_value)
                    new_state = {"mode": "hierarchy", "level": slider_value, "subprocess_id": None}
                    return fig, None, new_state, back_hidden, slider_visible, f"Showing hierarchy level {slider_value + 1}"
                else:
            ...
            if triggered_id == "main-graph" and click_data:
                ...
                if customdata:
                    ...
                    if sp and len(sp.nodes) > 1:
                        ...
                        fig = self._create_drilldown_figure(subprocess_id, current_level)
                        new_state = {
                            "mode": "drilldown", 
                            "level": current_level,
                        ...
                        return fig, no_update, new_state, back_visible, slider_hidden, info
